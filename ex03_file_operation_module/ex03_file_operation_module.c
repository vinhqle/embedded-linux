...
static int device_init(void){
	...
	// Create device class
	if (IS_ERR(cl = class_create(THIS_MODULE, "ex03_file_operation_class")))
	{
		unregister_chrdev_region(first, 1);
		return PTR_ERR(cl);
	}
	if (IS_ERR(dev_ret = device_create(cl, NULL, first, NULL, "ex03_file_operation_device")))
	{
		class_destroy(cl);
		unregister_chrdev_region(first, 1);
...
static ssize_t device_write(struct file *f, const char __user *buf, size_t len, loff_t *off)
{
	...
	copy_from_user(recvBuf, buf, len);
	if(recvBuf[0] == '1'){
